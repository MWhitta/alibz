#!/usr/bin/env python3
"""Install a hash-pinned Pantheum retrieval bundle on Moissanite; dry run by default.

The manifest pins existing source and private configuration. Only its listed
files and acquire.retrieval/opal_retrieval change. No instrument command runs.
"""
import argparse
import hashlib
import json
...
def inspect_bundle(bundle, root, config):
    ...
    patch = manifest['acquire_patch']
    if set(patch) != {'retrieval', 'opal_retrieval'} or patch['retrieval'] != 'opal_database':
        raise ValueError('Unexpected configuration patch')
    updated = json.loads(config.read_text())
    updated.setdefault('acquire', {}).update(patch)
    return manifest, updated


def check_idle(database):
    checks = {
        'acquisitions': "SELECT COUNT(*) FROM acquisitions WHERE state IN ('queued','running','cancelling')",
        'analysis jobs': "SELECT COUNT(*) FROM jobs WHERE status IN ('queued','running')",
...
def atomic_copy(source, target):
    ...
    tmp = target.with_name(target.name + '.ingest-tmp')
...
def install(bundle, root, config, state, apply=False, run=subprocess.run):
    ...
    with (state / 'reservation.lock').open('a+b') as lock:
        ...
        result = {'apply': apply, 'files': list(manifest['files']),
                  'retrieval': 'opal_database', 'active_work': 0}
        if not apply:
            return result
        backup = root.parent / ('pantheum-ingest-backup-' + datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S%fZ'))
        backup.mkdir(mode=0o700)
        # Stop under the physical-operation lock, then recheck the queue: no
        # acquisition can dispatch while it is held, and no worker remains.
        ...
        try:
            ...
            tmp = config.with_name(config.name + '.ingest-tmp')
