...
from alibz.utils.voigt import voigt_width as _voigt_width, multi_voigt as _multi_voigt
...
class PeakyFitter():
    ...
    def multi_voigt(self, x, *params):
        return _multi_voigt(x, np.asarray(params))
        ...
        for key in list(keys):
            for i, (element, ion, line_wavelength, distance, peak_match, peak_probability, absolute_intensity, *rest) in enumerate(self.finder.indexer.peak_probability_dictionary[key][:1]):
                ...
                try:
                    ...
                    ax2.set_ylabel(r'|$\epsilon_{line} / \epsilon_{calculated}$|')
...
class BaselineFinder():
    ...
    def calculate_baseline(self, x, y, peaks, kernel_width, plot=False):
        """Calculate an intensity baseline function to subtract from data.
        ...
        kernel_width : int
            Width of the uniform smoothing kernel.
        plot : bool, optional
